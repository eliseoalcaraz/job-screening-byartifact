...
export default function Navbar() {
  ...
  const getDropdownData = (category: any) => {
      ...
      <div className="hidden md:flex space-x-10 items-center">
        {navLinks.map((item) => {
          if (item.category === "window") {
            return <Dropdown key={item.label} title={item.label} data={windowsData} />;
          }
          if (item.category === "door") {
            return <Dropdown key={item.label} title={item.label} data={doorsData} />;
          }
          return (
            <a
              key={item.label}
